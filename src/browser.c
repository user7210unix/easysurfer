...
static void apply_css(BrowserData *data) {
    ...
    const char *css = data->dark_mode ?
        "box, notebook header { background: rgba(30, 30, 47, 0.8); padding: 8px; border: none; backdrop-filter: blur(10px); }"
        "entry { border: none; border-radius: 10px; padding: 8px 12px; background: rgba(51, 51, 68, 0.9); color: #fff; }"
        "button { background: rgba(51, 51, 68, 0.9); border: none; border-radius: 8px; padding: 6px 8px; color: #fff; transition: background 0.2s; }"
        "button:hover { background: #0078d4; }"
        "notebook tab { background: rgba(51, 51, 68, 0.9); color: #ccc; padding: 6px 12px; border: 1px solid #0078d4; border-bottom: none; border-radius: 8px 8px 0 0; }"
        "notebook tab:checked { background: #0078d4; color: #fff; }" :
        "box, notebook header { background: rgba(240, 242, 245, 0.8); padding: 8px; border: none; backdrop-filter: blur(10px); }"
        "entry { border: none; border-radius: 10px; padding: 8px 12px; background: rgba(255, 255, 255, 0.9); color: #333; }"
        "button { background: rgba(229, 231, 235, 0.9); border: none; border-radius: 8px; padding: 6px 8px; color: #333; transition: background 0.2s; }"
        "button:hover { background: #0078d4; color: #fff; }"
        "notebook tab { background: rgba(229, 231, 235, 0.9); color: #666; padding: 6px 12px; border: 1px solid #0078d4; border-bottom: none; border-radius: 8px 8px 0 0; }"
        "notebook tab:checked { background: #0078d4; color: #fff; }";
...
static void on_load_changed(WebKitWebView *web_view, WebKitLoadEvent load_event, GtkWidget *label) {
...
static WebKitWebView* create_web_view(BrowserData *data) {
    ...
    // uBlock Origin-like ad-blocking via user content manager
...
static void create_new_tab(BrowserData *data) {
    ...
    g_signal_connect(web_view, "load-changed", G_CALLBACK(on_load_changed), label);
...
static void on_bookmark_menu_popup(GtkMenuButton *button, BrowserData *data) {
    GtkWidget *menu = gtk_menu_button_get_popup(GTK_MENU_BUTTON(data->bookmark_button));
    populate_bookmark_menu(data, menu);
